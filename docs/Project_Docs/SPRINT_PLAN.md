...
- **2026-06-20 — Offers feature completion (4 phased PRs).** Completing the half-built offers feature per `docs/bmad/working-notes/2026-06-20-offers-feature-completion-handoff.md`. Phases: (1) edit route, (2) per-tenant offer-letter template foundation + de-hardcode + CI guard, (3) real delivery via a transactional email provider abstraction around refactored `sendOffer` EF (no false success; Resend acceptable for MVP non-PHI email, AWS SES target for regulated/ePHI-capable delivery), (4) AI reconnect to fill the tenant template. Each ships as its own PR after review.
  - Phase 1 — edit route: Done and merged as PR #25. Adds missing `offers/:id/edit` route in `App.tsx`.
  - Phase 2 - per-tenant offer-letter template foundation: [~] PR #26 code approved with comments and awaiting CTO review/merge on branch `feat/offers-template-foundation`; verification complete with build/shared tests/static guards green; full lint remains blocked by pre-existing repo-wide lint debt and live RLS assertions skipped without local Supabase env keys. Adds tenant_settings template/signatory/company columns, real Settings UI persistence, render utility, explicit offer-settings consumption in offer preview/AI drafting, de-hardcoded offer/AI/sendOffer surfaces, and CI literal guard. Public `get_public_offer` is expiry-filtered and does not return `secure_token`, applicant email, or applicant phone. No `db push`, deploy, or Phase 3 delivery wiring.
  - Phase 3 - real delivery: [B] Blocked until explicit CTO approval after Phase 2 / PR #26 is merged. Scope must start with an email provider abstraction, then refactor `sendOffer` to send an existing offer by id, render/store the sent letter, and mark `Sent` only after provider acceptance. Do not add new Brevo-only offer delivery.
  - Phase 4 - AI reconnect to fill tenant template: [B] Blocked until explicit CTO approval after Phase 3.
